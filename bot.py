...
# 服务器IP、端口、RCON密码
host = '127.0.0.1'#"xy.frp.one"  # 地址
port = 25575 #22222  # 端口
password = "11451419191810"  # 密码
async def mc_send_cmd(cmd):
    if(mc_ping(host) == None):
        return("与服务器失去连接")
...
async def hello_world(msg:dict,websocket):
    ...
    print(f"[{year}.{month}.{day} - {hour}:{minute}:{second}] [INFO] 群消息{'<-' if str(m) == '3609822427' else '->'} [{'发送' if str(m) == '3609822427' else '接收'}]{mmsg}")
